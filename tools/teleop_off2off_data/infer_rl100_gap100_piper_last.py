...
def load_checkpoint(path: Path, device: torch.device) -> tuple[RL1002D, dict]:
...
def offline_smoke(args) -> None:
    ...
    history = deque([item, item], maxlen=OBS_STEPS)
...
def validate_args(args) -> None:
    ...
    if args.rate != 15.0 or args.camera_fps != 15:
        print(
            f"[频率警告] checkpoint在15Hz轨迹上训练，当前使用"
            f"control={args.rate:g}Hz camera={args.camera_fps}Hz；"
            "动作的实际时间尺度将随执行频率改变。",
            flush=True,
        )
    if args.max_camera_gap_ms > 100.0 or args.max_camera_gap_ms <= 0:
        raise ValueError("--max-camera-gap-ms必须在(0,100]内")
    if args.max_steps <= 0:
...
def run_robot(args) -> None:
    ...
    try:
        ...
        if args.execute:
            phrase = input("确认工作区安全后输入 EXECUTE GAP100 PIPER 继续：").strip()
            if phrase != "EXECUTE GAP100 PIPER":
        ...
        planner = JointTrajectoryPlanner(
            ...
            tracking_error=args.planner_tracking_error_rad,
        )
        ...
        for step in range(args.max_steps):
            ...
            if result is not None and result[0] != last_result_id:
                request_id, origin_time, chunk, infer_elapsed, _ = result
