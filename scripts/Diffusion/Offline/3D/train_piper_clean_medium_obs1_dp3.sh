#!/usr/bin/env bash
# Train a one-observation-frame medium DP3 baseline on the original clean dataset.
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/../../../.." && pwd)"
project_root="${repo_root}/RL-100"
dataset_path="data/piper_pick_and_place_augmented_chunk4_control_clean.zarr"
run_dir="data/outputs/piper_pick_and_place_augmented_chunk4_control_clean_dp3_medium_obs1_episode10_bs64_epoch4000_seed42"
status_dir="${project_root}/data/outputs/piper_pick_and_place_augmented_chunk4_control_clean_dp3_medium_obs1_status"
...
printf "%s start medium obs1 dataset=%s run_dir=%s\n" \
...
python train.py --config-name=rl100_3d_epsilon.yaml \
    ...
    exp_name=piper-pick-place-clean-medium-obs1-dp3-bs64 \
    ...
    horizon=4 n_obs_steps=1 n_action_steps=4 chunk_as_single_action=True \
